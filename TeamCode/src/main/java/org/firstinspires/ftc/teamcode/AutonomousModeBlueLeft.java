...
public class AutonomousModeBlueLeft extends OpMode {
    ...
    public void loop() {
        ...
        if (!hasMoved[0]) {
            ...
            hasMoved[5] = robot.StrafeByDistance(68, robot.GetYaw(), telemetry);
        } else if (!hasMoved[6]) {
            try {
            ...
            hasMoved[7] = robot.StrafeByDistance(68, robot.GetYaw(), telemetry);
        } else if (!hasMoved[8]) {
            try {
            ...
            hasMoved[9] = robot.StrafeByDistance(800, Math.PI/2+robot.GetYaw(), telemetry);
        } else if (!hasMoved[10]) {
           hasMoved[10] = true;
        } else {
            robot.SetStrafe(0,0);
